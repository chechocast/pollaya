				...
				<div id="clientsCarrusel" class="carousel slide" data-ride="carousel">
					<!--<ol class="carousel-indicators">
						<?php 
							...
							$totalClients = sizeof($clientes->posts);
							$totalPages = ceil($totalClients / 6);
							for ($iClient=0; $iClient < $totalPages; $iClient++) { 
							$activeStyle = $iClient == 0 ? 'active' : '';

						?>
							<li data-target="#clientsCarrusel" data-slide-to="<?=$iClient?>" class="<?=$activeStyle?>"></li>
						<?php } ?>
					</ol>-->
					<div class="carousel-inner">
						<?php
						...
						<?php 
							if($iClient % 6 == 0 || $totalClients == $iClient)
							{
								?>
								</div>
								<?php
